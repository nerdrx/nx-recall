...
import { voiceStateLabel } from './views/voice.js';
...
async function refresh(){
  ...
  try{
    ...
    const audioText=audio?`Attached · ${route?.playback ?? 0} incoming stream(s), ${route?.capture ?? 0} microphone stream(s)`:
      (snapshot.running?'Waiting for audio. Check the selected devices or join Vesktop voice. Typed requests can still work.':'Stopped. Start Lanalu to connect audio.');
    const rows=[['Models',snapshot.available?'Ready on this computer':'Setup required'],['Audio',audioText],
      ...
      ['Latest state',voiceStateLabel({running:snapshot.running,available:snapshot.available,error:snapshot.lastError,status:{event:snapshot.state}})],
    ...
    events.replaceChildren(...snapshot.events.slice(-12).reverse().map(event=>h('li',{text:`${new Date(event.at).toLocaleTimeString()} · ${event.event.replaceAll('_',' ')}`})));
    if(!snapshot.events.length)events.append(h('li',{text:'No activity observed yet.'}));
    data.textContent=JSON.stringify(snapshot,null,2);status.textContent=`Updated ${new Date().toLocaleTimeString()}`;
  }
