# Technical reference and development history

[← NX Recall](../README.md) · [Current Local Voice guide](LOCAL-VOICE.md) · [Changelog](../CHANGELOG.md)

This preserves the previous README in full, including installation, build
commands, experiments, measurements and the historical development log.
Descriptions and measurements reflect the stage at which they were written;
they are not universal accuracy guarantees or a current feature matrix.
For current setup and behavior, start with the [main README](../README.md).
In particular, Local Voice can now speak retrieved memory into a Discord call;
older statements below about having no sharing surface predate that feature.
