...
* **0.11.0, Japanese** — the decoder does not speak the language and does not
  say so. Below.
* **0.11.6, the other languages** — the decoder speaks the language perfectly
...
`sherpa-onnx-nemo-parakeet-tdt_ctc-0.6b-ja-35000-int8@1`, and `text_via: "lid"`
— its own value since 0.11.4, where it was previously `"arbiter"`. A row that
says `arbiter` was a German or English flip caught by reading the text; this is
a different claim about the audio.
...
### The other-languages half (0.11.6)
...
`es`, `it`; `pt`, `nl` and `pl` are available by naming them) the turn is
