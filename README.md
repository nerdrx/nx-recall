...
| 0.12.0 | +127h | speaker recognition re-measured on 1,150 of Discord's own verdicts: a voice is scored on whether its record agrees rather than on its luckiest prototype (wrong labels −62%); a learned space the evidence had already refused was still installed and is now taken back; `identity repair --prototypes` throws out voiceprints that are recordings of somebody else; a better extractor, a PLDA and every presence prior measured and refused, with the reasons written down |
