...
pub fn calibrate(
    ...
    {
        let mut best: Option<(calib::Aggregate, Score, Score)> = None;
        for a in calib::aggregate_grid() {
            if a == base_agg {
                continue;
            }
            let (s, sm) = judge(cfg, &globals, &bank, &eval, None, you, a)?;
            if best
                .as_ref()
                .is_none_or(|(_, b, _)| s.f_beta(calib::BETA) > b.f_beta(calib::BETA))
            {
                best = Some((a, s, sm));
            }
        }
        if let Some((a, s, sm)) = best {
