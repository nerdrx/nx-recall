...
//! [`crate::asr_cjk`] fixes. Japanese fails because Parakeet-TDT-0.6b-v3 does
...
//!
//! [`crate::asr_cjk::judge`] is safe almost for free, and it is worth being
...
//!    [`crate::asr_cjk::pre_route`] — shared, not re-implemented — sends a turn
...
//! difference between this route and the Japanese one. `asr_cjk`'s arbiter
...
///
/// Deliberately the **same** `lid` value [`crate::asr_cjk`] writes, rather than a
...
pub const LANG_VIA_LID: &str = crate::asr_cjk::LANG_VIA_LID;
...
/// with a WER gate behind it (+62.4% / +31.2% relative at 1.0 / 1.5 s, 0 rows
/// made worse). `es` and `it` had the other half measured — LID recall of 89.0%
/// and 58.5% at 1.5 s, the same false-positive behaviour, the identical judge —
/// but not the half that says the replacement is better than what it
/// overwrites, and half a measurement is not the gate. They stay in
/// [`ROUTABLE`] and out of the default until a GPU evening collects their rows;
/// `polyglot_languages = ["fr", "es", "it"]` turns them on by hand. `pt`, `nl`
/// and `pl` are in [`ROUTABLE`] and not here because nothing about them was
/// measured at all.
pub const MEASURED: &[&str] = &["fr"];
...
///
/// Returns the tag to re-decode into. Separate from [`crate::asr_cjk::post_route`]
...
/// the reason [`crate::arbiter::judge`] and [`crate::asr_cjk::judge`] both are:
...
/// session.
///
/// Holds no identifier: it reads the reading [`crate::asr_cjk`] already paid
/// for.
pub struct Polyglot {
    enabled: bool,
    night: NightModels,
...
mod tests {
    ...
    fn the_allowlist_is_the_config_and_the_catalogue_together() {
        ...
        // In the catalogue, not in the shipped default: Spanish had half its
        // measurement (identification) and Dutch none, so an operator has to
        // ask for either by name.
        assert!(is_routable("es"));
    ...
    fn the_identifier_has_to_name_a_language_this_route_can_act_on() {
        ...
        // Italian is routable but not in the measured default, so the shipped
        // config declines it and an explicit allowlist takes it.
        assert_eq!(post_route(Some(&reading("it", 1.0)), &cfg), None);
        let wide = AsrConfig {
            polyglot_languages: vec!["fr".into(), "es".into(), "it".into()],
            ..AsrConfig::default()
        };
        assert_eq!(
            post_route(Some(&reading("it", 1.0)), &wide),
            Some("it".into())
        );
        ...
        // Japanese belongs to `asr_cjk`, which runs FIRST and has already dealt
    ...
    fn a_row_this_route_settles_looks_exactly_like_one_the_japanese_route_did() {
        ...
        assert_eq!(LANG_VIA_LID, crate::asr_cjk::LANG_VIA_LID);
    ...
    fn the_shipped_operating_point_is_the_measured_one() {
        ...
        // the same number until 0.11.8 and they answer different questions —
        ...
        assert!(cfg.polyglot);
        // French only: the one language with a WER gate behind it.
        assert_eq!(cfg.polyglot_languages, ["fr"]);
        assert_eq!(cfg.polyglot_languages, MEASURED);
    }
