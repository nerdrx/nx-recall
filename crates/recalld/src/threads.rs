...
pub fn assign(
    ...
    let Some((session_id, bridges, turn)) = store.segment_turn(segment_id)? else {
        return Ok(None);
    };
    ...
    // session made every thread a monologue (2026-09-02). The room mic (0.10.0)
    // bridges on the same rule: the room and the headset are one evening.
    let scope = if bridges { None } else { Some(session_id) };
