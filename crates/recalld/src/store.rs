...
// ---- 0.8.0, notes to self (schema v10) ----------------------------------
// v10 adds `notes` — a mic turn that opened with a wake phrase, filed as
// something you asked to be reminded of. One table, one migration, no
// backfill: nothing said before the feature existed was addressed to it.
...
impl Store {
    ...
    fn init(&self) -> Result<()> {
        ...
        // v10 (0.8.0): notes to self. Standalone like v9 — one table that
        // references `segments` and nothing else, and no backfill.
        self.apply_v10()?;
...
impl Store {
    /// Everything schema v10 adds, written so it is a no-op on a v10 database.
