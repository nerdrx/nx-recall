...
pub struct AsrConfig {
    ...
    /// `ROUTABLE` and deliberately so: `fr` has a WER gate behind it, `es` and
    /// `it` have the identification and false-positive halves of the same
    /// measurement, and `pt`/`nl`/`pl` have nothing. All six are reachable by
    /// naming them here, and the honest thing is that the last three are
    /// untested rather than unavailable.
