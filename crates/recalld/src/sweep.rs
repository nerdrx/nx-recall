...
//! (FINDINGS §29), and the third is the round's actual finding.
//!
...
/// replaced** by the sweep's own (FINDINGS §29).
///
...
pub struct Pass<'a> {
    ...
    /// (FINDINGS §29). With it off the sweep asks the identifier and writes
...
pub fn sweep_one(
    ...
    // `[asr].lang_sweep_redecode` (FINDINGS §29).
    //
...
mod tests {
    ...
    fn the_sweeps_floor_is_its_own_and_is_never_below_the_live_one() {
        // The measured narrowing (FINDINGS §29). A sweep floor an operator sets
    ...
    fn the_sweep_ships_writing_a_language_and_never_a_word() {
        // FINDINGS §29, and the whole shape of the round. The routes' 0.97%
